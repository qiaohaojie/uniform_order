 ...
 *
 * See demo/demo_data/README.md and demo/demo_data/operator_run_guide.md.
 */
...
function checkSafety(databaseUrl: string, flags: Flags) {
  ...
  try {
    ...
    abortWithGuard(
      ...
      "Set DATABASE_URL in demo/demo_data/.env.demo to a postgres connection string."
    );
  }

  // Localhost and Neon cloud hosts are allowed by default. Other remotes need --allow-remote.
  // Prod Neon project is still blocked below via PROD_HOST_PATTERNS.
...
/** Number of academy orders attributed to DEMO_PARENT_USER_ID when set. */
const PARENT_ATTRIBUTED_ORDER_COUNT = 3;

/**
 * Wipe one demo tenant's rows (same scope as cleanup-demo.ts).
 * Used by --reset before re-seeding.
 */
async function wipeDemoTenant(db: Db, tenantId: string) {
  await db
...
async function seedTenant(
  ...
  if (t.orders.length > 0) {
    await seedOrders(db, t, legalVersionId, ctx);
  }
  void flags;
}
...
async function seedOrders(
  ...
  for (const o of t.orders) {
    ...
    // Attribute the first N orders to the demo parent so /orders history works.
    const userId =
      ctx.parentUserId && attributed < PARENT_ATTRIBUTED_ORDER_COUNT
        ? (attributed++, ctx.parentUserId)
        : null;
  ...
  const attributedMsg =
    attributed > 0
      ? `, ${attributed} attributed to DEMO_PARENT_USER_ID`
      : "";
  console.log(`  ✓ orders (${t.orders.length}${attributedMsg})`);
}

async function resolveParentUserId(db: Db): Promise<string | null> {
  const raw = process.env.DEMO_PARENT_USER_ID?.trim();
  if (!raw) {
    console.log(
      "  · DEMO_PARENT_USER_ID unset — parent order history relies on parent_email match only.",
    );
    return null;
  }
  if (!UUID_RE.test(raw)) {
    console.warn(
      `  ! DEMO_PARENT_USER_ID is not a valid UUID (${raw}) — ignoring.`,
    );
    return null;
  }
  const lookup = await db.execute<{ id: string }>(
    sql`SELECT id FROM neon_auth."user" WHERE id = ${raw} LIMIT 1`,
  );
  if (!lookup.rows[0]?.id) {
    throw new Error(
      `DEMO_PARENT_USER_ID=${raw} not found in neon_auth."user". ` +
        "Create the parent Neon Auth user first, or clear DEMO_PARENT_USER_ID.",
    );
  }
  console.log(
    `  · DEMO_PARENT_USER_ID set — first ${PARENT_ATTRIBUTED_ORDER_COUNT} orders per tenant with orders will use userId ${raw.slice(0, 8)}…`,
  );
  return raw;
}
...
async function main() {
  ...
  if (!databaseUrl) {
    abortWithGuard(
      ...
      "Copy demo/demo_data/.env.demo.example to demo/demo_data/.env.demo and fill DATABASE_URL."
    );
  }
  ...
  if (flags.dryRun) {
    console.log("\n[DRY RUN] Would seed:");
    if (flags.reset) {
      console.log("  --reset: wipe existing demo tenant rows first");
    }
    for (const t of wantedTenants) {
      console.log(`  tenant ${t.id} — ${t.catalog.length} items, ${t.orders.length} orders`);
    }
    if (process.env.DEMO_PARENT_USER_ID?.trim()) {
      console.log(
        `  DEMO_PARENT_USER_ID set — would attribute up to ${PARENT_ATTRIBUTED_ORDER_COUNT} orders per tenant`,
      );
    }
  ...
  if (flags.reset) {
    console.log("\n→ --reset: wiping existing demo data for selected tenants");
    for (const t of wantedTenants) {
      await wipeDemoTenant(db, t.id);
      console.log(`  ✓ wiped ${t.id}`);
    }
  }

  const parentUserId = await resolveParentUserId(db);
  const ctx: SeedContext = { parentUserId };

  for (const t of wantedTenants) {
    console.log(`\n→ Seeding tenant ${t.id} (${t.name})`);
    await seedTenant(db, t, flags, ctx);
  }
