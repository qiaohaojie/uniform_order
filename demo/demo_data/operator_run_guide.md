...
4. Optionally: open the Neon dashboard, navigate to the Auth tab, copy the UUID of the parent account, and paste it into `.env.demo` as `DEMO_PARENT_USER_ID`. Re-running the seed attributes the first 3 orders (per tenant with orders) to that `user_id` so parent portal history matches by user id as well as email. If left unset, orders still use `parent_email=parent@demo.uniformorder.online` (list/detail match by email when signed in as that parent).
...
**Fastest local path (development only):** open

`http://localhost:3000/api/dev/login?email=operator@demo.uniformorder.online&callbackURL=/admin/demo-academy`

This sets a dev session cookie without Neon Auth. It does not create `neon_auth.user` rows — seed still needs at least one real auth user for legal versions.
...
## Targeting a Neon or other remote DB

**Neon cloud hosts (`*.neon.tech`) are allowed by default** — use the same `DATABASE_URL` as `apps/web/.env.local`. The production Neon project remains blocked.

For non-Neon remotes only:

```bash
pnpm --filter web demo:seed -- --allow-remote
```

Always run `--dry-run` first when pointing at a new host.
