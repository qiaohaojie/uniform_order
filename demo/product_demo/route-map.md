...
| `/orders/[orderId]` | parent (own orders) | Parent order detail page | sign in as parent email (or set `DEMO_PARENT_USER_ID` for user_id attribution) | Optional Act 3 closing shot |
...
- **`/orders/[orderId]`** allows access when the session email matches `orders.parentEmail` (seeded as `parent@demo.uniformorder.online`), or when `orders.userId` matches. Set `DEMO_PARENT_USER_ID` to attribute the first 3 orders by user id as well.
