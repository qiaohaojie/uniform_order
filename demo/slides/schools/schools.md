...
- **No overstating.** If a feature is on the roadmap, the slide says "on the roadmap". The "Closing claims" table in `demo/product_demo/product-walkthrough.md` is the source of truth — every claim on every slide must be traceable to a ✓ live row, or labelled as 🚧 planned / positioning.
...
**Imagery.**
- Use real product screenshots from `demo/product_demo/recordings/` (or the demo environment at the seeded `demo-blank` / `demo-academy` tenants) wherever a slide references a feature.
...
- **Speaker note:** This is the slide where you take out your own phone and walk through the live demo if presenting in person. Reference `demo/product_demo/demo-playbook.md` Acts 1–3.
...
- Reorder sections.
- Invent statistics. Every figure must come from `demo/product_demo/product-walkthrough.md` § 8 or be labelled as "in pilot / to be validated".
...
Read § 8 of `demo/product_demo/product-walkthrough.md` before drafting. The following claims are **safe** for this deck because every one is ✓ live:
