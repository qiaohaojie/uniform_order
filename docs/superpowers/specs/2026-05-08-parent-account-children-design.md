...
- **Picker stays public — and doubles as the public school chooser.** Signed-in users see their saved children plus an "Add another child" button. Not-signed-in users see the publicly-listed tenants as crest cards (so they can still find their school without a deep-link), with a "Sign in to save your children" CTA below. Catalog browsing remains anonymous-friendly to match today's flow. See "Picker flow" below for the full UI contract.
...
- **Read:** server-side via `cookies()` from `next/headers` in the routes that personalise the *in-progress* shopping experience: `/[tenant]/page.tsx` (catalog banner), `/[tenant]/cart/page.tsx` (cart banner via prop to its client component), and `/[tenant]/checkout/page.tsx` (form prefill). The order-placed page is **not** a cookie consumer — its source of truth is the placed order row, looked up by `searchParams.orderId`; see the cleanup map below. Reading helper: `getActiveChild()` in a new `lib/active-child.ts` — reads cookie, ownership-checks against `getSessionUser()`, returns `{ id, name, year, rollClass, tenantId } | null`.
...
| **Integration (Playwright)** | Mock Neon Auth session via cookie injection. Cover: signed-out picker renders publicly-listed tenants as a school chooser → tapping a tenant card navigates to its catalog → "Sign in to save your children" CTA → sign-in callback resumes add-child sheet → create / list / edit / remove → tap-saved-child sets `uo:active-child` cookie and opens prefilled checkout → stale-year prompt visible after fast-forwarding system date past Jan 1 → tenant filter hides un-listed tenants in the sheet → un-listed tenants are not present on the logged-out chooser either. |
