...
- [ ] **Pre-existing variant misalignments.** Investigation against the paper form (`my_doc/UI_prototypes/project/uploads/Uniform_Online_Order_Form.pdf`) showed a row of mis-attributions in the seed/CATALOG predating PR #12: the rows the prior `remaining_work.md` flagged as "missing shirt variants" actually belong to other products. Specifically: `White Shirt SS` should have only `10–26 → $32` (the "Mens 4–8 → $43" row was the Navy Shorts variant); `White Shirt LS` should have only `10–24 → $28` (the "Mens 5–8 → $59" row was the Trousers variant); `Trousers` should be `10–18 → $57` and `Mens 5–8 → $59` (the four `Year X–Y short/long` rows were the School Tie variants); `School Tie` should be the four `Year 7–10 / Year 11–12 × short/long` length rows ($17/$18) (the existing `One size → $20` was the Scarf price). Fix these in a small focused PR — does **not** touch RGSH, Stripe products/prices, or the schema.
