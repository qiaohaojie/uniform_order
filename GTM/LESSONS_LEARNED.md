...
- `DATABASE_URL` (in `GTM/demo_data/.env.demo`) must point at a localhost host *or* you need to pass `--allow-remote` to the seed script. The safety guard is good but the error wording (`SAFETY GUARD TRIPPED`) is easy to misread as a different kind of failure.
