...
**The accent colour should switch per school deployment.** When the deck is generated for a specific school, the gold accent above can be replaced with the school's `accent_colour` token (e.g. NSBH navy, RGSH crimson) read from the tenant settings. If no school is specified, default to gold.
